<template>
  <div class="app-container">
    <el-header style="text-align: right; font-size: 12px">
      <el-input @keyup.enter.native="handleFilter" style="width: 200px;" class="filter-item" placeholder="名称搜索" v-model="listQuery.title">
      </el-input>
       <el-select clearable class="filter-item" value-key="name" placeholder="音乐艺术家" v-model="listQuery['artistsMusic.id']" filterable>
              <el-option v-for="t in artists.content" :key="t.name" :label="t.name" :value="t.id">
              </el-option>
        </el-select>
       <el-select clearable class="filter-item" value-key="name" placeholder="视觉艺术家" v-model="listQuery['artistsVision.id']" filterable>
              <el-option v-for="t in artists.content" :key="t.name" :label="t.name" :value="t.id">
              </el-option>
        </el-select>
      ...
      border fit highlight-current-row>
        ...
        <el-table-column label="发布时间">
          <template slot-scope="scope">
            {{scope.row.publishedAt | moment('YYYY-MM-DD')}}
          </template>
        </el-table-column>
        <el-table-column
          prop="hidden"
...
<!--          </template>-->
<!--        </el-table-column>-->
        <el-table-column label="艺术家（音乐）">
          <template slot-scope="scope">
            {{scope.row.artistsMusic && scope.row.artistsMusic.length ? scope.row.artistsMusic.map(i=>i.name).reduce((a,b)=>a+' '+b) : ''}}
          </template>
        </el-table-column>
        <el-table-column label="艺术家（视觉）">
          <template slot-scope="scope">
            {{scope.row.artistsVision && scope.row.artistsVision.length ? scope.row.artistsVision.map(i=>i.name).reduce((a,b)=>a+' '+b) : ''}}
          </template>
        </el-table-column>
